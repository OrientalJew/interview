...
将n个元素的无序序列分割为n个有序的序列\(只有一个元素的情况下，认为是有序的\)，然后开始两两合并这些有序序列。合并的过程对它们进行排序，使之成为新的有序序列。接着，重复上述步骤，不断的的两两合并为新的有序序列，直到最后只剩下一个有序序列，排序完成。

时间复杂度为O\(NlogN\)，一般认为是稳定的算法。

#### 基数排序

也叫桶排序，和堆排序一样，都是基于一种数据解构类型而发明 的排序算法。
