...
* 当数据量达到阀值时，每次扩容原容量1/2，创建新的数组，将元素都拷贝进新的数组中；
...
* 实现的数据结构是链表\(循环链表，双向链表\)，则增删的效率高；但是查找的效率低，因为它需要先定位到目标结点\(要拿到当前结点，必须先拿到它的前一个结点\)；
* 对查找是做了优化的，查找时会根据index先判断目标结点是位于链表的前半段还是后半段，然后才决定是进行顺序查询还是逆序查询；
* 在Android中，对LinkedList进行修改，实现了Queue接口，提供了对应的入队和出队方法，并且实现的数据解构是循环链表结构，通过一个空结点voidLink指向了队列的头结点和尾结点，形成了循环结构，并且能够快速的找到头尾结点；

#### HashMap

* HashMap是基于散列链表：数组+单链表
* HashMap中数据的存储方式是通过其中的table数组长度减1，再与要存储的数据的hash值进行逻辑与运算，计算出存储数据在table数组中的下标值。需要注意的是，这个下标下的数组元素可能已经存储了数据，当两个数据的key相同，则会用新数据代替旧数据，否则**出现了Hash冲突**，存储数据被插入到对应下标的链表的头部；
...
但是，数组的空间太大，势必会带来空间利用率的减低，空间太小，则出现Hash冲突的几率就增大了。为了解决这个问题，HashMap中设置了一个阀值\(负载因子\)，其默认值为0.75。
...
扩容实际就是创建一个为原来空间大小两倍的数组，再将旧数组的数据移动到新数组中；
...
> 对于k层的完全二叉树，其k-1层为满二叉树，而k层均为叶子结点，对于前k-1层有
>
> $$m=2^{k-1}-1 $$ \(m表示前k-1层结点数\)
>
> $$2^{k-1}=m+1$$
>
> $$k=log_2^{m+1} +1$$
>
> 则完全二叉树的深度为小于它一层的满二叉树的深度+1；
>
> 对于有n个结点的满二叉树的深度为$$log_2^{n+1}$$，而大于它一层的完全二叉树的结点个数至少为n+1个，则满二叉树的深度+1就是完全二叉树的深度；

* 前序遍历、中序遍历、后序遍历？

前序遍历：先根节点，然后左子树节点，如果左子树节点存在子节点，则继续先遍历左子树节点，然后才是遍历右子树节点；

中序遍历：先遍历根节点的左子节点，如果左子节点也存在左子节点，则继续先遍历该左子节点，然后才是父节点，接着才是右子结点；

后序遍历：先遍历左子节点，然后是右子结点，之后才是父结点；

层序遍历：从根节点第一层开始，每一层从左向右遍历结点；
...
3、如果父结点是红色的，则子节点不能是红色的，即相邻的两个结点不能都是红色的；

4、叶子结点总是黑色的；

5、从同一结点向叶子结点出发的两个路径上，拥有的黑色结点数是相同的；

* 二叉树增删的核心思想

TreeMap的插入操作，无非就是先进行遍历比较，找到合适的插入点进行插入，然后再进行平衡化操作\(左旋、右旋、颜色反转\)即可，因为新插入的结点总是红色的，所以我们只需要进行适当的左旋、右旋和颜色反转，让红黑树最终符合其5大原则即可；

比较复杂的是删除操作，该过程最终也是回到平衡化的过程中；删除操作需要先找到删除结点的前驱或者后继结点，然后用这个结点的内容覆盖到删除结点上\(不包括颜色\)，这样即可完成删除结点的删除，并且不会影响到删除处的平衡；

接着问题就变成了删除原先的前驱或后继结点；由于前驱或后继结点处于分支末端，所以删除难度较小；

如果是红色的，直接删除即可；如果是黑色的，则删除后会影响到所在分支的平衡，我们需要对红黑树进行重新调整；

再平衡的过程：

因为删除了一个黑色结点，则其所在分支少了一个黑色结点，为了让红黑树不失衡，我们假设代替删除结点的子节点多了一个黑色的结点，然后在不断往上遍历，将黑色结点推给父亲结点，如果父亲结点是红色的，那么直接将父亲结点置黑\(相当于父亲结点消耗了黑色结点\)，如果父节点是黑色的，则继续往上推，直到找到能够消耗掉黑色结点的红色结点为止。

在黑色结点往上推的过程中，因为兄弟分支会多出一个黑色结点，所以兄弟分支需要进行相应的颜色调整，来维持平衡。
